...
class CVRPModel(nn.Module):
    ...
    def forward(self, state, theta, cur_dist, ins_feature, weights=None, selected=None, return_probs=False):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder(encoded_last_node, state.load, cur_dist, theta, ins_feature, ninf_mask=state.ninf_mask, weights=weights)
...
class CVRP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, load, cur_dist, theta, ins_feature, ninf_mask, weights=None):
