...
class CVRPTrainer:
    ...
    def __init__(self,
        ...
        if self.meta_params["data_type"] == "size_distribution":
            ...
            self.min_n, self.max_n, self.task_interval, self.num_dist = 100, 501, 10, 11
            self.task_w = torch.full(((self.max_n - self.min_n) // 10 + 1, self.num_dist), 1 / self.num_dist)
    ...
    def _train_one_epoch(self, epoch):
        ...
        for b in range(self.meta_params['B']):
            if self.meta_params["data_type"] == "size_distribution":
                ...
                batch_size = int(meta_batch_size * (100 / task_params[0])**1.6)
        ...
        for b in range(self.meta_params['B']):
            ...
            for step in range(self.meta_params['k']):
                ...
                env_params = {'problem_size': data[-1].size(1), 'pomo_size': data[-1].size(1), 'device': self.device}
    ...
    def _train_one_batch(self, task_model, data, env, optimizer=None):
        ...
        while not done:
            theta, cur_dist, ins_feature = env.get_local_feature()
            selected, prob = task_model(state, theta, cur_dist, ins_feature)
            # shape: (batch, pomo)
        ...
        # shape: (batch, pomo)
        loss = loss / advantage.max(dim=1)[0][:, None]
        loss_mean = loss.mean()
    ...
    def _train_one_batch_maml(self, fast_weight, data, env, optimizer=None, create_graph=True):
        ...
        while not done:
            theta, cur_dist, ins_feature = env.get_local_feature()
            selected, prob = self.meta_model(state, theta, cur_dist, ins_feature, weights=fast_weight)
            # shape: (batch, pomo)
            state, reward, done = env.step(selected)
            prob_list = torch.cat((prob_list, prob[:, :, None]), dim=2)
    ...
    def _fast_val(self, model, data=None, path=None, offset=0, val_episodes=32, mode="eval", return_all=False, bootstrap_model=None):
        ...
        if mode == "eval":
            ...
            with torch.no_grad():
                ...
                while not done:
                    theta, cur_dist, ins_feature = env.get_local_feature()
                    selected, _ = model(state, theta, cur_dist, ins_feature)
                    # shape: (batch, pomo)
                    state, reward, done = env.step(selected)
            ...
            while not done:
                if mode == "maml":
                    theta, cur_dist, ins_feature = env.get_local_feature()
                    selected, prob, probs = self.meta_model(state, theta, cur_dist, ins_feature, weights=fast_weight, return_probs=True)
                    if bootstrap_model is not None:
                        probs1 = torch.where(probs > 0, probs, torch.tensor(0.00001))
                        with torch.no_grad():
                            _, _, bs_probs = self.meta_model(state, theta, cur_dist, ins_feature, weights=bootstrap_model, selected=selected, return_probs=True)
                            bs_probs = torch.where(bs_probs > 0, bs_probs, torch.tensor(0.00001))
                else:
                    theta, cur_dist, ins_feature = env.get_local_feature()
                    selected, prob, probs = model(state, theta, cur_dist, ins_feature, return_probs=True)
                    if bootstrap_model is not None:
                        probs1 = torch.where(probs > 0, probs, torch.tensor(0.00001))
                        with torch.no_grad():
                            _, _, bs_probs = bootstrap_model(state, theta, cur_dist, ins_feature, selected=selected, return_probs=True)
                            bs_probs = torch.where(bs_probs > 0, bs_probs, torch.tensor(0.00001))
