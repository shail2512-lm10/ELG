...
class CVRPTester:
    def __init__(self,
        ...
        if tester_params['test_set_path'].endswith(".pkl"):
            ...
            env_params = {'problem_size': len(self.test_data[0][-2]), 'pomo_size': len(self.test_data[0][-2]), "device": self.device}
    ...
    def _test_one_batch(self, test_data):
        ...
        while not done:
            theta, cur_dist, ins_feature = self.env.get_local_feature()
            with torch.no_grad():
                selected, _ = self.model(state, theta, cur_dist, ins_feature)
            # shape: (batch, pomo)
    ...
    def _fine_tune_one_batch(self, fine_tune_data):
        ...
        while not done:
            theta, cur_dist, ins_feature = self.env.get_local_feature()
            selected, prob = self.model(state, theta, cur_dist, ins_feature)
            # shape: (augmentation * batch, pomo)
            state, reward, done = self.env.step(selected)
