...
def solve_lkh_log(executable, directory, name, depot, loc, demand, capacity, grid_size=1, runs=1, disable_cache=False, MAX_TRIALS=20000):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-vrplib", action='store_true', help="Set true to test on vrplib instances. ")
    ...
    for dataset_path in opts.datasets:
        ...
        if opts.vrplib:
            ...
            for name in files:
                ...
                if instance['dimension'] > 500 and instance['dimension'] <= 1001:
                # if instance['dimension'] > 0 and instance['dimension'] <= 200:
                    scales.append(instance['dimension'])
                    instance = ((instance['node_coord'][instance['depot']] / 1000.0).tolist(), (instance['node_coord'][1:]/ 1000.0).tolist(), 
                                instance['demand'][1:].tolist(), instance['capacity'])
                    # print(instance)
                    optimal.append(solution['cost'])
        ...
        if opts.vrplib:
            ...
            for i in range(len(costs)):
                # if optimal[i] == 154593 or optimal[i] == 146332:
                #     print(i)
                #     continue
                if scales[i] <= 200:
                    small_gap.append((1000 * costs[i] - optimal[i]) / optimal[i])
                elif scales[i] <= 500:
                    medium_gap.append((1000 * costs[i] - optimal[i]) / optimal[i])
                elif scales[i] <= 1001:
                    large_gap.append((1000 * costs[i] - optimal[i]) / optimal[i])
                print(costs[i], optimal[i])

            # print(scales)
            print(len(large_gap))
            print("Small average gap: {:.2f}%".format(100 * (np.array(small_gap).mean())))
            print("Medium average gap: {:.2f}%".format(100 * (np.array(medium_gap).mean())))
            print("Large average gap: {:.2f}%".format(100 * (np.array(large_gap).mean())))
            print("Total average gap: {:.2f}%".format(100 * ((1000 * np.array(costs) - np.array(optimal)) / np.array(optimal)).mean()))
