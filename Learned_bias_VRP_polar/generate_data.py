...
def generate_vrp_data(dataset_size, problem_size, random_capacity=True):
    if random_capacity == False:
        ...
        data = {
            ...
            'depot': torch.FloatTensor(dataset_size, 2).uniform_(0, 1)
        }

    else:
        # According to the X-set of VRPLib
...
if __name__ == "__main__":
    data_size = 1000
    problem_size = 1000
    val_filename = f'data/vrp{problem_size}_val.pkl'
    test_filename = f'data/vrp{problem_size}_test.pkl'

    # generate validation data
    validation_dataset = VRPDataset(size=problem_size, num_samples=data_size)
