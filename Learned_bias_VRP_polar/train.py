...
def validate(model, multiple_width, device):
    ...
    # validation dataset
    val_100 = VRPDataset('data/vrp100_val.pkl', num_samples=5000)
    val_100_loader = DataLoader(val_100, batch_size=500)
    val_200 = VRPDataset('data/vrp200_val.pkl', num_samples=5000)
    val_200_loader = DataLoader(val_200, batch_size=500)
    val_500 = VRPDataset('data/vrp500_val.pkl', num_samples=100)
    val_500_loader = DataLoader(val_500, batch_size=20)
    val_1000 = VRPDataset('data/vrp1000_val.pkl', num_samples=100)
    val_1000_loader = DataLoader(val_1000, batch_size=20)
    ...
    val_1000_cost = test_rollout(val_1000_loader, env, model)

    avg_cost_list = [val_100_cost, val_200_cost, val_500_cost, val_1000_cost]
    return avg_cost_list
    

def train(model, train_steps, inner_steps, train_batch_size, problem_size, fixed_size, multiple_width, 
keep_num, lr, device, alg, logger, fileLogger, dir_path, log_step):
    ...
    for i in trange(train_steps):
        ...
        if fixed_size == 'varying':
            ...
            # true_batch_size = int(train_batch_size * (problem_size / true_problem_size)**0.7)
            # print(true_problem_size, true_batch_size)
        elif fixed_size == 'fixed':
            # fixed problem size of training samples
            true_problem_size = problem_size
            true_batch_size = train_batch_size
        elif fixed_size == 'rule_based':
            if i <= 200000:
                 # fixed problem size of training samples
