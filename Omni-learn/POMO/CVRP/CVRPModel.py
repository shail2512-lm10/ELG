...
class CVRPModel(nn.Module):
    ...
    def forward(self, state, theta, cur_dist, ins_feature, weights=None, selected=None, return_probs=False):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder(encoded_last_node, theta, cur_dist, ins_feature, state.load, ninf_mask=state.ninf_mask, weights=weights)
...
class CVRP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, theta, cur_dist, ins_feature, load, ninf_mask, weights=None):
        ...
        if weights is None:
            ...
            # shape: (batch, pomo, problem)
            if self.model_params['bias'] == 'learn':
                score_scaled += self.edge_net(theta, cur_dist, ins_feature)
